class Solution(object):
    def rob(self, nums):
        ...
        for i in range(3,nums_length - 1):
            max_sum.append(max(nums[i] + max_sum[i-2],nums[i] + max_sum[i-3]))
        first_max = max(max_sum[nums_length-2],max_sum[nums_length-3])
        max_sum = []
        max_sum.append(-1000)
        max_sum.append(nums[1])
        max_sum.append(nums[2])
        for i in range(3,nums_length):
            max_sum.append(max(nums[i] + max_sum[i-2],nums[i] + max_sum[i-3]))
        second_max = max(max_sum[nums_length-1],max_sum[nums_length-2])
